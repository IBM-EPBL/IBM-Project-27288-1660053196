...
def removepd():
    ...
    for x in data["listofhubs"]:
        if x["HubName"]==hubname:
          for y in x["ProductDetails"]:
              if y["productname"]==prname:
                  x["ProductDetails"].remove(y)
                  break
    print(data)
...
def removeSupplier(name):
    ...
    for x in data["listofhubs"]:
        if x["HubName"]==hubname:
            for t in x["SupplierDetails"]:
                if t["suppliername"] == supname:
                    x["SupplierDetails"].remove(t)
                    break
